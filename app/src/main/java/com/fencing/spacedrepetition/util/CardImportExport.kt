...
object CardImportExport {
    ...
    private const val HEADER_MARKER = "#FSR_EXPORT_V1"

    // Column indices for full export format
    private const val COL_QUESTION = 0
    private const val COL_ANSWER = 1
    private const val COL_ALGORITHM = 2
    private const val COL_NEXT_REVIEW = 3
    private const val COL_LAST_REVIEW = 4
    private const val COL_FSRS_STABILITY = 5
    private const val COL_FSRS_DIFFICULTY = 6
    private const val COL_FSRS_STATE = 7
    private const val COL_FSRS_REPS = 8
    private const val COL_FSRS_LAPSES = 9
    private const val COL_FSRS_SCHEDULED_DAYS = 10
    private const val COL_FSRS_ELAPSED_DAYS = 11
    private const val COL_SM2_EASE_FACTOR = 12
    private const val COL_SM2_INTERVAL = 13
    private const val COL_SM2_REPETITIONS = 14
    private const val COL_GROUPS = 15
    private const val FULL_COLUMN_COUNT = 16

    // Column headers for export (describes each field)
    private const val COLUMN_HEADERS = "#Question\tAnswer\tAlgorithm\tNextReview\tLastReview\t" +
     ...
     * Supports both simple (question\tanswer) and full export formats.
    ...
    fun parseCards(inputStream: InputStream): Pair<List<ParsedCard>, List<String>> {
        ...
        try {
            ...
            // Check if this is a full export format
            val isFullFormat = lines.firstOrNull()?.startsWith(HEADER_MARKER) == true
            val dataLines = if (isFullFormat) lines.drop(1) else lines

            dataLines.forEachIndexed { index, line ->
                val lineNumber = if (isFullFormat) index + 2 else index + 1
                ...
                try {
                    val parsedCard = if (isFullFormat) {
                        parseFullFormatLine(trimmedLine, lineNumber)
                    } else {
                        parseSimpleLine(trimmedLine, lineNumber)
                    }
    ...
    private fun parseFullFormatLine(line: String, lineNumber: Int): ParsedCard? {
        ...
        val question = unescapeNewlines(parts.getOrNull(COL_QUESTION)?.trim() ?: "")
        val answer = unescapeNewlines(parts.getOrNull(COL_ANSWER)?.trim() ?: "")
        ...
        // Parse full format
        val algorithmStr = parts.getOrNull(COL_ALGORITHM)?.trim() ?: "FSRS"
        val algorithm = try {
        ...
        val groupsStr = parts.getOrNull(COL_GROUPS)?.trim() ?: ""
        ...
        return ParsedCard(
            ...
            lineNumber = lineNumber,
            algorithm = algorithm,
            nextReview = parts.getOrNull(COL_NEXT_REVIEW)?.toLongOrNull() ?: 0L,
            lastReview = parts.getOrNull(COL_LAST_REVIEW)?.toLongOrNull() ?: 0L,
            fsrsStability = parts.getOrNull(COL_FSRS_STABILITY)?.toDoubleOrNull() ?: 0.0,
            fsrsDifficulty = parts.getOrNull(COL_FSRS_DIFFICULTY)?.toDoubleOrNull() ?: 0.0,
            fsrsState = parts.getOrNull(COL_FSRS_STATE)?.trim() ?: "NEW",
            fsrsReps = parts.getOrNull(COL_FSRS_REPS)?.toIntOrNull() ?: 0,
            fsrsLapses = parts.getOrNull(COL_FSRS_LAPSES)?.toIntOrNull() ?: 0,
            fsrsScheduledDays = parts.getOrNull(COL_FSRS_SCHEDULED_DAYS)?.toIntOrNull() ?: 0,
            fsrsElapsedDays = parts.getOrNull(COL_FSRS_ELAPSED_DAYS)?.toIntOrNull() ?: 0,
            sm2EaseFactor = parts.getOrNull(COL_SM2_EASE_FACTOR)?.toDoubleOrNull() ?: 2.5,
            sm2Interval = parts.getOrNull(COL_SM2_INTERVAL)?.toIntOrNull() ?: 0,
            sm2Repetitions = parts.getOrNull(COL_SM2_REPETITIONS)?.toIntOrNull() ?: 0,
            groupNames = groupNames
        )
    }

    /**
     * Exports cards with their groups to full format TSV
     */
